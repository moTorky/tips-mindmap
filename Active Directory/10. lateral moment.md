...
```
- Over Pass the hash (OPTH) -> generate tokens from hashes or keys (GET TGT from HASH). 
	...
	```
	**Notice:**
	- When using **RC4-HMAC**, the Kerberos key is **identical** to the user’s **NTLM hash**.
	    - This allows an attacker who has the NTLM hash to request a TGT if RC4 is enabled—this attack is known as **Overpass-the-Hash (OPtH)**.
	- When using **AES128/AES256**, the key is **derived** from the user's password via:
	    1. **PBKDF2-HMAC-SHA1** with **salt** (usually realm + principal name) and ~**4096 iterations**.
	    2. A Kerberos-specific **"DK" function** using the constant string `"kerberos"` 
	    This derived **AES key** is what Kerberos uses to:
	    - Encrypt the **timestamp** in the AS-REQ (PA-ENC-TIMESTAMP).
	    - Decrypt and verify it on the KDC side.
	🔐 **Implication:** You _cannot_ use the NTLM hash directly with AES encryption—the NTLM hash does _not_ produce the needed AES key. As a result:
		- **OPtH only works with RC4.**
		- To abuse AES enctypes, an attacker must **dump the actual AES128/AES256 key** from LSASS (using tools like Mimikatz), then perform a **Pass-the-Key** style attack using that key—not OPtH.
		https://chatgpt.com/c/684a9385-3e48-8005-bc35-6a9212c50eeb
		https://web.mit.edu/kerberos/krb5-devel/doc/admin/enctypes.html?utm_source=chatgpt.com
		https://redfoxsec.com/blog/attacking-kerberos-part-1/
